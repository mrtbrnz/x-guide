...
class Guidance(object):
    def __init__(self, verbose=False, interface=None, quad_ids = None):
        ...
        print('Rotorcrafts :', self.rotorcrafts)
        self.connect = pprz_connect.PprzConnect(notify=new_ac, verbose=False)
        self._interface = self.connect.ivy
        time.sleep(0.5)

        for rc in self.rotorcrafts:
            rc.sm = settings.PprzSettingsManager(self.connect.conf_by_id(rc.id).settings, rc.id, self.connect.ivy)
