...
def parse_msg(sender_id, msg):
    if msg.name == 'ROTORCRAFT_FP':
        # if self.verbose: 
        print('ROTORCRAFT_FP received',msg.get_field(0), msg.get_field(1), msg.get_field(2) )
        print('north : ', msg['north'])
        # update rotorcraft class 
        rotorcraft_fp_cb(sender_id,msg)
    # self.set_accel_data(msg)

    # print("new message from %i: %s" % (sender, msg))
...
def test():
    ...
    serial_interface = SerialMessagesInterface(parse_msg, device=args.dev,
